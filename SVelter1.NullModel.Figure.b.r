...
if (StatType=='RDNull'){
    ...
    hist(data_RD,xaxt='n',xlab='Read Depth',ylab='Frequency',freq=F,cex.main=cex_main,main='Fitted by NB',col=boxcolor,breaks=break_num)
    lines(seq(range(data_RD)[1],range(data_RD)[2],1),dnbinom(seq(range(data_RD)[1],range(data_RD)[2],1),r,p),type='l',lwd=2,col=linecolor)
...
if (StatType=='ILNull'){
    ...
    xrange=range(min(data_IL)*0.9,max(data_IL)*1.1)
...
if (StatType=='TBNull'){
    ...
    xin=c(1:dataNull[nrow(dataNull),1])
    yin=mixmdl$lambda[1] * dnorm(xin,mean=mixmdl$mu[1],sd=mixmdl$sigma[1])+mixmdl$lambda[2] * dnorm(xin,mean=mixmdl$mu[2],sd=mixmdl$sigma[2])
    xrange=range(min(data_TB)*0.9,max(data_TB)*1.1)
    yrange=range(0,max(yin)*1.2)
    par(mfrow=c(1,2))
    par(mar=rep(3,4))
    par(fig=c(0.1,0.6,0,1))
