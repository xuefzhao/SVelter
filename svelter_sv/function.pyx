...
def bed_hash_short(hash_in,Chromo_Length):
    out={}
    for k1 in list(hash_in.keys()):
        if hash_in[k1]==[[]]:
            if k1 in list(Chromo_Length.keys()):
                out[k1]=[[0,Chromo_Length[k1]]]
        else:
            temp_hash={}
            for k2 in hash_in[k1]:
                if len(k2)>1:
                    if not int(k2[0]) in list(temp_hash.keys()):
                        temp_hash[int(k2[0])]={}
                    if not int(k2[1]) in list(temp_hash[int(k2[0])].keys()):
...
def basic_block_RD(Initial_block_RD,original_letters,Af_Letter):
    ...
    for m in after_letter:
        if not m in list(blockCN.keys()):
            blockCN[m]=1
        elif m in list(blockCN.keys()):
            blockCN[m]+=1
    Af_rd=[]
    for n in Af_Letter:
        Af_rd.append([])
...
def BPList_Insert_Pool_Sequence(BP_List_origin,Ref_Sequence_Path_File,flank):
    ...
    seq_ori=''.join(seq1)
    relative_List=[a-BP_List_origin[0]+flank for a in BP_List_origin]
...
def Compare_LN_list_against_Segdup_list(segdup_list,LN_list,chromo_name):
    ...
    rec2=0
    while True:
        if not chromo_name in list(segdup_list.keys()): break
        ...
        if LN_list[rec1][1]<segdup_list[chromo_name][rec2][0]:
            ...
            rec1+=1
    if rec1<len(LN_list):
        in_list+=list(range(len(LN_list)))[rec1+1:]
...
def calculate_len_genome(ref):
    ...
    len_genome=0
    for i in list(whole_genome.keys()):
...
def clusterNum2(data1,data2,direction,numCff):
    ...
    if len(datatemp)>0:
        ...
        if direction=='f':
            while True:
                if not data1[flag2] <datatemp[flag1][0] and not data1[flag2] >datatemp[flag1][1]:
                    if not datatemp[flag1][1] in list(out.keys()):
        ...
        if direction=='r':
            while True:
                if not data1[flag2] <datatemp[flag1][0] and  not data1[flag2] >datatemp[flag1][1]:
                    if not datatemp[flag1][0] in list(out.keys()):
...
def clusterNums4(hash, ClusterLen,direction):
    data1=clusterNums(list(hash.keys()),ClusterLen,direction)[0]
    if direction=='r':
        data1=data1[::-1]
    temp=[[] for i in data1]
    if direction=='f':
...
def clusterNumSP(hash,ClusterLen,numCff):
    ...
    while True:
        if data[flag2]< out2[0][flag1]:
            ...
            if not out1[0][flag1] in list(out.keys()):
...
def clusterNumLN(hash,ClusterLen,numCff):
    ...
    for key in list(hash.keys()):
        ...
        for key3 in hash[key]:
            if not key3[1] in list(key2.keys()):
                key2[key3[1]]=key3[0]
            else:
                key2[key3[1]]+=key3[0]
        for key4 in list(key2.keys()):
                ...
                if key4=='ff' or key4=='rf':
                    ...
                    for temp2 in range(len(temp1[1])):
                        if temp1[1][temp2]>numCff:
                            if not key4 in list(out[key].keys()):
...
def clusterLN(data, LnHash, numCff):
    out={}
    for i in data:
        if not i in list(LnHash.keys()):
            continue
        else:
            j=LnHash[i]
            key=j[0]+j[2]
            if not key in list(out.keys()):
                out[key]=[j[1]]
            else:
                out[key]+=[j[1]]
        ...
        if flag1==len(dataRou): break
            ...
            if flag2==len(dataCen)-1:
                if dataRou[flag1]>dataCen[flag2]+ClusterLen: break
                    ...
                    if not dataCen[flag2] in list(out.keys()):
                ...
                if flag1==len(dataRound): break
                    ...
                    if flag2==len(dataLeft)-1 and dataRound[flag1]>dataRight[flag2]: break
                        ...
                        if dataRound[flag1]<dataLeft[flag2]:
                            ...
                            if not dataLeft[flag2] in list(out.keys()):
                ...
                if flag1==len(dataRound): break
                    ...
                    if flag2==len(dataLeft)-1 and dataRound[flag1]>dataRight[flag2]: break
                        ...
                        if dataRound[flag1]<dataLeft[flag2]:
                            ...
                            if not dataRight[flag2] in list(out.keys()):
...
def cn2_length_readin(dict_opts):
    if '--null-copyneutral-length' in list(dict_opts.keys()):
...
def Direction_Penalize_2a(Coordinate_Info):     
    ...
    Direction_Penalty=0
    for key in list(Coordinate_Info.keys()):
...
def DR_Penal_Calcu(read_info):
    DR_Penal=0
    for i2 in list(read_info[2].keys()):
        for i in read_info[2][i2]:
            if i[4:6]==['+', '+'] or i[4:6]==['-', '-'] or i[4:6]==['-', '+']:
                DR_Penal+=1
    for i2 in list(read_info[3].keys()):
...
def GC_Stat_ReadIn(BamN,GC_Stat_Path,genome_name,affix):
    ...
    f_GC_stat.close()
    for k1 in list(CN2_Region.keys()):
        for k2 in list(CN2_Region[k1].keys()):
...
def genome_name_readin(dict_opts):
    if not '--NullGenomeName' in list(dict_opts.keys()):
        genome_name='genome'
    else:
...
def GC_Index_Readin(fileGC):
    ...
    while True:
        ...
        if not pin[0] in list(out.keys()):
            out[pin[0]]={}
        if not int(pin[1]) in list(out[pin[0]].keys()):
            out[pin[0]][int(pin[1])]={}
        if not int(pin[2]) in list(out[pin[0]][int(pin[1])].keys()):
...
def GC_RD_Adj_hash(GC_Median_Num,GC_Overall_Median_Num,Chromo,GC_Content,Coverage):
    ...
    for key_1 in list(GC_Content.keys()):
            Be_Adj_RD=Coverage[key_1]
            GC_Con=int(round(GC_Content[key_1]*100))
            if GC_Con in list(GC_Median_Num.keys()):
...
def GC_RD_Info_Complete(ref_file,GC_Median_Coverage,ChrN_Median_Coverage,GC_Overall_Median_Coverage,GC_Var_Coverage,GC_Mean_Coverage,GC_Std_Coverage,Chromosome):
    ...
    for x in [chrom_N,chrom_X,chrom_Y]:
        if not x in list(GC_Median_Coverage.keys()):
            GC_Median_Coverage[x]={}
        for i in list(ChrN_Median_Coverage.keys()):
            GC_Median_Coverage[x][i]=numpy.mean(ChrN_Median_Coverage[i])
    for x in [chrom_N,chrom_X,chrom_Y]:
        GC_Overall_Median_Coverage[x]=numpy.mean([GC_Overall_Median_Coverage[key] for key in list(GC_Overall_Median_Coverage.keys())])
        GC_Var_Coverage[x]=numpy.mean([GC_Var_Coverage[key] for key in list(GC_Var_Coverage.keys())])
        GC_Mean_Coverage[x]=numpy.mean([GC_Mean_Coverage[key] for key in list(GC_Mean_Coverage.keys())])
        GC_Std_Coverage[x]=numpy.mean([GC_Std_Coverage[key] for key in list(GC_Std_Coverage.keys())])
    for x in Chromosome:
        if not x in list(GC_Median_Coverage.keys()):
            GC_Median_Coverage[x]={}
        for i in list(ChrN_Median_Coverage.keys()):
            GC_Median_Coverage[x][i]=numpy.mean(ChrN_Median_Coverage[i])
        if not x in list(GC_Overall_Median_Coverage.keys()):
            GC_Overall_Median_Coverage[x]=GC_Overall_Median_Coverage[chrom_N]
        if not x in list(GC_Var_Coverage.keys()):
            GC_Var_Coverage[x]=GC_Var_Coverage[chrom_N]
        if not x in list(GC_Mean_Coverage.keys()):
            GC_Mean_Coverage[x]=GC_Mean_Coverage[chrom_N]
        if not x in list(GC_Std_Coverage.keys()):
...
def IL_Stat_Calculate(InsertLength):
    TotalILNum=0
    for key in list(InsertLength.keys()):
...
def IL_Penal_Calcu(read_info,IL_Statistics,Cut_Upper,Cut_Lower,Penalty_For_InsertLengthZero):
    Initial_IL=[]
    for j2 in list(read_info[2].keys()):
        for j in read_info[2][j2]:
            Initial_IL.append(j[3]-j[1])
    for j2 in list(read_info[3].keys()):
...
def LN_file_readin(LN_filein):
    ...
    for line in fin:
        pin=line.strip().split()
        if not int(pin[1]) in list(LN_hash.keys()):
            LN_hash[int(pin[1])]={}
        if not int(pin[3]) in list(LN_hash[int(pin[1])].keys()):
...
def LNd_hash_readin(chrom_LN,bps_in_path):
    ...
    for line in fin:
        ...
        if not '_'.join([pin2[0],pin2[3]]) in list(LNd_hash.keys()):
            tempkey='_'.join([pin2[0],pin2[3]])
            LNd_hash[tempkey]=[{},{}]
        if not pin2[1] in list(LNd_hash[tempkey][0].keys()):
            LNd_hash[tempkey][0][pin2[1]]=[]
        LNd_hash[tempkey][0][pin2[1]]+=pin2[4:6]
        if not pin2[2] in list(LNd_hash[tempkey][0].keys()):
            LNd_hash[tempkey][0][pin2[2]]=[]
        LNd_hash[tempkey][0][pin2[2]]+=pin2[4:6]
        if not pin2[4] in list(LNd_hash[tempkey][1].keys()):
            LNd_hash[tempkey][1][pin2[4]]=[]
        LNd_hash[tempkey][1][pin2[4]]+=pin2[1:3]
        if not pin2[5] in list(LNd_hash[tempkey][1].keys()):
            LNd_hash[tempkey][1][pin2[5]]=[]
        LNd_hash[tempkey][1][pin2[5]]+=pin2[1:3]
    fin.close()
    for k1 in list(LNd_hash.keys()):
        for k2 in list(LNd_hash[k1][0].keys()):
            if k2 in ['f','r','+','-']:
        ...
        for k2 in list(LNd_hash[k1][1].keys()):
            if k2 in ['f','r','+','-']:
    ...
    for k1 in list(LNd_hash.keys()):
        ...
        for k2 in list(LNd_hash[k1][0].keys()):
            temp2=[k2]
            for k3 in LNd_hash[k1][0][k2]:
                for k4 in list(LNd_hash[k1][0].keys()):
        ...
        for k2 in list(LNd_hash[k1][1].keys()):
            temp2=[k2]
            for k3 in LNd_hash[k1][1][k2]:
                for k4 in list(LNd_hash[k1][1].keys()):
        ...
        for k2 in tempall[k1]:
            if k2[0]==k1.split('_')[0]:
                for k3 in k2[1:]:
                    ...
                    if k3 in list(LNd_hash[k1][0].keys()):
                        del LNd_hash[k1][0][k3]
            if k2[0]==k1.split('_')[1]:
                for k3 in k2[1:]:
                    if k3 in k1.split('_'): break
                    if k3 in list(LNd_hash[k1][1].keys()):
                        del LNd_hash[k1][1][k3]
        temp2=[]
        clu1=clusterNums_bpintegrate(sorted([int(i) for i in list(LNd_hash[k1][0].keys())]), 1000, 'f')
        cen1=clusterQC_bpintegrate(clu1, 2)
        if not cen1==[]:
            sup1=clusterSupVis_bpintegrate(sorted([int(i) for i in list(LNd_hash[k1][0].keys())]),cen1,1000)
            for k3 in list(sup1.keys()):
                ...
                if not temp3 in temp2:
                    temp2.append(temp3)
        clu2=clusterNums_bpintegrate(sorted([int(i) for i in list(LNd_hash[k1][1].keys())]), 1000, 'f')
        cen2=clusterQC_bpintegrate(clu2, 2)
        if not cen2==[]:
            sup2=clusterSupVis_bpintegrate(sorted([int(i) for i in list(LNd_hash[k1][1].keys())]),cen2,1000)
            for k3 in list(sup2.keys()):
...
def LNd_hash_pair(LNd_hash,SP_Info):
    out={}
    for k1 in list(LNd_hash.keys()):
        if k1.split('_')[0] in list(SP_Info.keys()) and k1.split('_')[1] in list(SP_Info.keys()):
            out[k1]=[]
            cen1=[int(i) for i in list(LNd_hash[k1][0].keys())]
            if not cen1==[]:
                clu1=clusterSupVis_bpintegrate(sorted(SP_Info[k1.split('_')[0]].keys()),cen1,10000)
                for k2 in cen1:
                    if k2 in list(clu1.keys()):
                        temp=[]
                        for k3 in clu1[k2]:
                    ...
                    if not k3==[]:
                        ...
                        for k4 in k3:
                            if k4 in list(clu2.keys()):
                                ...
                                clu2[k4]=k4+100
                        LNd_hash[k1][0][k2]=[list(clu2.keys())[0],clu2[list(clu2.keys())[0]]]
            if not cen1==[]:
                for k2 in list(clu1.keys()):
...
def LN_bps_write(bps_hash,bps_folder,S_Sample,dict_opts,chromos,allchromos,bps_in_path):
    ...
    SP_Info=SP_LI_in[1]
    SP_Check=SP_LI_in[2]
    S_Chr=list(bps_hash[S_Sample].keys())[0] 
                    ...
                    for x in LN_out4:
                        ...
                        for y in x:
                            ...
                            for z in y:
                                ...
                                if not z[0] in list(test.keys()):
                                    test[z[0]]=[]
                                test[z[0]]+=z[1:]
                            for z in list(test.keys()):
...
def left_RD_Calculate_2a(Through_GCRD_Adj,Af_GCRD_Adj,flank,Window_Size):
    left_blocks=list(range(flank/Window_Size+1))[1:]
...
def link_SP_link3(SP_link3,SP_Info,min_length):
    out={}
    for k1 in list(SP_link3.keys()):
        temp2=[]
        temp1={}
        for k2 in SP_link3[k1]:
            if not k2[0] in list(temp1.keys()):
                temp1[k2[0]]=[]
...
def Move_Choice_procedure_2(Move,Letter_List,Letter_List_origin,ChrAllele):
    ...
    if len(Letter_List)==0 and not Move=='insert':
        ...
        if Move=='delete':
            ...
            Move_Pool2=[]
            insert_p=random.choice(list(range(len(Letter_List)+2))[1:])
...
def Move_Decide_2(IL_List,RD_List,GC_Var_Coverage):
    ...
    if sum(T2_Penal)>0:
        Normalized_Penal=[l/numpy.sum(T2_Penal) for l in T2_Penal]
        indicator=float(random.choice(list(range(1000))))/1000
...
def multi_dup_define(letter_RD,GC_Overall_Median_Coverage):
    ...
    Multi_Dup=[]
    for x in list(letter_RD.keys()):
        for y in list(letter_RD[x].keys()):
        ...
        while True:
            ...
            if temp3[rec2][-1]-temp3[rec2][0]<10**7:
                if temp1[rec1]<temp3[rec2][0]-10**4:
                    ...
                    if not '_'.join([str(temp3[rec2][0]),str(temp3[rec2][-1])]) in list(temp4.keys()):
...
def NullPath_SetUp(out_path,dict_opts):
    if '--out-path' in list(dict_opts.keys()):
...
def original_bp_let_produce(chr_letter_bp,bps2):
    ...
    for i in bps2[1:]:
        temp_ks=[]
        for k2 in list(chr_letter_bp[i[0]].keys()):
...
def out_pair_bp_check(out_pair_bp,link_limit):
    ...
    out_hash={}
    for k1 in out:
        if not k1[0] in list(out_hash.keys()):
            out_hash[k1[0]]=[]
        if not k1[1] in list(out_hash.keys()):
            out_hash[k1[1]]=[]
        out_hash[k1[0]].append(k1[1])
        out_hash[k1[1]].append(k1[0])
    for x in list(out_hash.keys()):
...
def SP_Info_Cluster(SP_Info):
    for S_Chr in list(SP_Info.keys()):
...
def SP_info_ReadIn_pre(bps_hash,S_Sample,bps_in_path):
    single_chromos=list(bps_hash[S_Sample].keys())
    SP_links={}
    SP_Link1={}
    SP_Info={}
    SP_Link2={}
    SP_Link3={}
    for k1 in list(bps_hash[S_Sample].keys()):
        filein1=bps_hash[S_Sample][k1][0]
        fin=open(bps_in_path+filein1)
        for line in fin:
            pin=line.strip().split()
            if not pin[0] in list(SP_links.keys()):
                SP_links[pin[0]]={}
            if not pin[0] in list(SP_Link1.keys()):
                SP_Link1[pin[0]]=[]
            if not pin[0] in list(SP_Info.keys()):
                SP_Info[pin[0]]={}
            if not int(pin[1]) in list(SP_Info[pin[0]].keys()):
                SP_Info[pin[0]][int(pin[1])]=int(pin[2])
            else:
                SP_Info[pin[0]][int(pin[1])]=max([int(pin[2]),SP_Info[pin[0]][int(pin[1])]])
            if not int(pin[3]) in list(SP_Info[pin[0]].keys()):
    ...
    for S_Chr in list(SP_Link1.keys()):
        SP_Link2[S_Chr]=[int(i) for i in SP_Link1[S_Chr]]
    for S_Chr in list(SP_Link1.keys()):
        bpCluster=clusterSupVis_bpintegrate(list(SP_Info[S_Chr].keys()),SP_Link2[S_Chr],10)
        SP_Link3[S_Chr]=[]
        for k1 in list(bpCluster.keys()):
            if len(bpCluster[k1])==1 and bpCluster[k1][0]==k1:
                del bpCluster[k1]
        for k1 in range(len(SP_Link2[S_Chr])):
            k2=SP_Link2[S_Chr][k1]
            if k2 in list(bpCluster.keys()):
...
def SP_LN_info_ReadIn(LN_list,all_SPs,LN_filein,SP_filein):
    ...
    for line in fin:
        ...
        if not int(pin[1]) in list(all_SPs[pin[0]].keys()):
            ...
            all_SPs[pin[0]][int(pin[1])]=max([all_SPs[pin[0]][int(pin[1])],int(pin[2])])
    fin.close()         
    for k1 in list(LN_Links.keys()):
        if not k1 in list(LN_list.keys()):
            LN_list[k1]=[]
...
def SP_Info_Merge(SP_hash):
    ...
    while True:
        ...
        for x in min_index:
            if SP_pos[x] in list(SP_hash.keys()) and SP_pos[x+1] in list(SP_hash.keys()):
        ...
        for y in x:
            ...
            for x in temp[1]:
                if not x in list(temp2.keys()):
                    ...
                    temp2[x]+=1
            for x in temp[2]:
                if not x[0] in list(temp2.keys()):
                    temp2[x[0]]=1
                else:
                    temp2[x[0]]+=1
            for x in list(temp2.keys()):
...
def SPLCff_Calculate(NullSplitLen_perc,SPLenStat,ReadLength):
    if NullSplitLen_perc>1:
        ...
        if not list(SPLength.keys())==[]:
            for key in list(SPLength.keys()):
...
def pos_block_assign(block_bps_chr,read_pos,tolerance_bp):
    ...
    for i in read_new:
        if type(i)==type(1):
            for j in list(block_bps_chr.keys()):
                if j=='left':
...
def RDNullfigure2_Modify(RDNullfigure2,Window_Size):
    ...
    print(' '.join(pin1), file=fo)
    print(' '.join([str(i) for i in [float(j)/Window_Size for j in pin2]]), file=fo)
    fo.close()
...
def SplitLenPNum_Calculate(SplitLength,NullPath,bamF,bam_file_appdix,genome_name,NullSplitLen_perc):
    ...
    for key in sorted(SplitLength.keys()):
            print(' '.join([str(key),str(SplitLength[key])]), file=fslo)
...
def TBNullDensity_CleanUP(TBNullDensity):
    if 0 in list(TBNullDensity.keys()):
        del TBNullDensity[0]
    TotalILNum=0
    for key in list(TBNullDensity.keys()):
...
def write_bp_1a(LN_LN_Merge,bps_folder,chromo_name,S_Sample):
    ...
    for x in LN_LN_Merge:
        print(' '.join([chromo_name]+[str(i) for i in sorted(numpy.unique(x))]), file=fo)
        print(' ', file=fo)
    fo.close()
...
def write_bp_3a(LN_LN_Merge,bps_folder,file_length,chromo_name,S_Sample):
    ...
    for x in out_list:
        ...
        fo=open(fout,'w')
        for y in x:
            print(' '.join([chromo_name]+[str(i) for i in sorted(numpy.unique(y))]), file=fo)
            print(' ', file=fo)
        fo.close()
def Write_filtered_LN_list(LN_keep_list,LN_filein,chromo_name):
    fo=open(LN_filein,'w')
    for k1 in LN_keep_list:
        print(' '.join([str(i) for i in [chromo_name,k1[0],k1[2],k1[1],k1[3]]]), file=fo)
    fo.close()
def Write_filtered_SP_list(SP_keep_list,SP_filein,chromo_name):
    fo=open(SP_filein,'w')
    for x in SP_keep_list:
        print(' '.join([str(i) for i in [chromo_name]+x]), file=fo)
    fo.close()
...
def bed_writing(filename,hash,chromos):
    fo=open(filename,'w')
    for k3 in chromos:
        if k3 in list(hash.keys()):
            for k4 in hash[k3]:
                print(' '.join([str(i) for i in k4]), file=fo)
    fo.close()
...
def dup_collaps(dup1):
    out={}
    for k1 in list(dup1.keys()):
        out[k1]={}
        for k2 in dup1[k1]:
            if not k2[0] in list(out[k1].keys()):
                out[k1][k2[0]]={}
            if not k2[1] in list(out[k1][k2[0]].keys()):
                out[k1][k2[0]][k2[1]]=[]
...
def dup_collaps_refine(out):
    ...
    for k1 in list(out.keys()):
        ...
        for k2 in out3:
            ...
            for k3 in rec_t2:
                if not k3[0] in list(outout[k1].keys()):
                    outout[k1][k3[0]]={}
                if not k3[1] in list(outout[k1][k3[0]].keys()):
                    outout[k1][k3[0]][k3[1]]=[]
...
def dup_collaps_refine2(out):
    ...
    for k1 in list(out2.keys()):
        for k2 in list(out2[k1].keys()):
            for k3 in list(out2[k1][k2].keys()):
...
def GC_RD_Adj(GC_Median_Num,GC_Overall_Median_Num,Chromo,GC_Content,Coverage):
    ...
    for key_1 in range(len(GC_Content)):
            ...
            if GC_Con in list(GC_Median_Num.keys()):
                    Median_Coverage=GC_Median_Num[GC_Con]
                    Af_Adj_RD=Be_Adj_RD*Overall_Median_Coverage/Median_Coverage
            elif not GC_Con in list(GC_Median_Num.keys()):
                    Median_Coverage=Overall_Median_Coverage
                    Af_Adj_RD=Be_Adj_RD*Overall_Median_Coverage/Median_Coverage
...
def RD_within_B_calcu(GC_Mean_Coverage,Full_Info,bps2):
    ...
    for i in list(RD_within_B.keys()):
...
def total_rd_calcu(GC_Median_Num,GC_Overall_Median_Num,letter_RD2,letter_GC,chr_letter_bp,block_rd2):
    out={}
    for k1 in list(block_rd2.keys()):
        for k2 in list(block_rd2[k1].keys()):
            if not k2 in ['left','right']:
                out[k2]=[]
    for k2 in list(letter_RD2.keys()):
...
def write_VCF_header(output_file,time,workdir):
    ...
    print('##INFO=<ID=BKPTID,Number=.,Type=String,Description="ID of the assembled alternate allele in the assembly file">', file=fo)
    ...
    print('##INFO=<ID=HOMLEN,Number=.,Type=Integer,Description="Length of base pair identical micro-homology at event breakpoints">', file=fo)
    print('##INFO=<ID=HOMSEQ,Number=.,Type=String,Description="Sequence of base pair identical micro-homology at event breakpoints">', file=fo)
    print('##INFO=<ID=IMPRECISE,Number=0,Type=Flag,Description="Imprecise structural variation">', file=fo)
    print('##INFO=<ID=MEINFO,Number=4,Type=String,Description="Mobile element info of the form NAME,START,END,POLARITY">', file=fo)
    print('##INFO=<ID=SVLEN,Number=.,Type=Integer,Description="Difference in length between REF and ALT alleles">', file=fo)
    print('##INFO=<ID=SVTYPE,Number=1,Type=String,Description="Type of structural variant">', file=fo)
    print('##INFO=<ID=SVCLASS,Number=1,Type=String,Description="Classification of structural variant">', file=fo)
    ...
    print('\t'.join(['#CHROM','POS','ID','REF','ALT','QUAL','FILTER','INFO','FORMAT',output_file.split('/')[-1].replace('.vcf','')]), file=fo)
    fo.close()
def sv_reorganize_reorder(sv_reorganize):
    out={}
    for k1 in list(sv_reorganize.keys()):
        out[k1]={}
        for k2 in list(sv_reorganize[k1].keys()):
            for k3 in list(sv_reorganize[k1][k2].keys()):
                for k4 in sv_reorganize[k1][k2][k3]:
                    if not k4[1] in list(out[k1].keys()):
                        out[k1][k4[1]]={}
                    if not k4[2] in list(out[k1][k4[1]].keys()):
                        out[k1][k4[1]][k4[2]]=[]
        ...
        for k2 in list(sv_out[k1].keys()):
            ...
            for k3 in range(len(SVtemp_a)):
                for k4 in SVtemp_b[k3]:
                    ...
                    if not SVtemp_info[0] in list(sv_reorganize.keys()):
                        sv_reorganize[SVtemp_info[0]]={}
                    if not SVtemp_info[1] in list(sv_reorganize[SVtemp_info[0]].keys()):
                        sv_reorganize[SVtemp_info[0]][SVtemp_info[1]]={}
                    if not SVtemp_info[2] in list(sv_reorganize[SVtemp_info[0]][SVtemp_info[1]].keys()):
...
def ori_let_Modi(Be_Info, ori_let2,Copy_num_estimate):
    ...
    for x in Ab_Dir_L1_a:
        if not x[3] in list(vote_inv_hash.keys()):
            vote_inv_hash[x[3]]=1
        else:
            vote_inv_hash[x[3]]+=1
    for x in Ab_Dir_L1_b:
        if not x[4] in list(vote_inv_hash.keys()):
            vote_inv_hash[x[4]]=0.5
        else:
            vote_inv_hash[x[4]]+=0.5
        if not x[6] in list(vote_inv_hash.keys()):
            vote_inv_hash[x[6]]=0.5
        else:
            vote_inv_hash[x[6]]+=0.5
    for x in Ab_Dir_L2_a:
        if not x[0] in list(vote_inv_hash.keys()):
            vote_inv_hash[x[0]]=1
        else:
            vote_inv_hash[x[0]]+=1
    for x in Ab_Dir_L2_b:
        if not x[0] in list(vote_inv_hash.keys()):
            ...
            vote_inv_hash[x[0]]+=0.5
        if not x[2] in list(vote_inv_hash.keys()):
            vote_inv_hash[x[2]]=0.5
        else:
            vote_inv_hash[x[2]]+=0.5
    new_let2=[]
    for x in ori_let2:
        if x in list(vote_inv_hash.keys()) and vote_inv_hash[x]>2:
            new_let2.append(x+'^')
        else:
            new_let2.append(x)
    new_group=[]
...
def sv_info_score_modify(sv_info):
    all_scores_pos=0
    all_scores_neg=0
    for x in list(sv_info.keys()):
        for y in list(sv_info[x].keys()):
